...
```{r}
SantaRosa_final <- SantaRosa[order(SantaRosa$fecha),]
SantaRosa_final <- select(SantaRosa_final, fecha, TmaxC, TminC, Lluvia)
```
...
```{r}
SantaRosa_final$djuliano <- yday(SantaRosa_final$fecha)

SantaRosa_graficos <- SantaRosa_final %>% replace_na(list(Lluvia = 0))
...
ggplot(SantaRosa_graficos, aes(x = djuliano, y = LluviaAcumulada)) +
  geom_line(aes(group = factor(year(fecha))), size = 0.1)

```

Totales de precipitación por año:
```{r}
colnames(SantaRosa_graficos)[6] <- "Anno"
LluviaTotalAnno <- SantaRosa_graficos %>% group_by(Anno) %>% summarise(sum(Lluvia))
colnames(LluviaTotalAnno)[2] <- "TotalLluvia"

ggplot(LluviaTotalAnno, aes(Anno, TotalLluvia)) +
  geom_col()
...
ggplot(SantaRosa_final, year(fecha), TmaxC) +
  geom_boxplot()
```
